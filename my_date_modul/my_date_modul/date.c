...
PUBLIC Date* set_date_random(Date* ptr)
{
	...
	ptr->m_month = 3;
	ptr->m_year = rand() % (RANDOM_YEAR_MAX - RANDOM_YEAR_MÝN + 1 )  + RANDOM_YEAR_MÝN;
...
PUBLIC void set_date_array(Date* ptr, size_t size)
{
	while (size--)
		set_date_random(ptr++);
}
...
PUBLIC void print_date_array(const Date* ptr, size_t size)
{
	while (size--)
	{
		print_date(ptr++);
